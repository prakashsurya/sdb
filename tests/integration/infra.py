...
def get_command_output_filename(cmd: str) -> str:
    """
    Given an SDB command in string form, generate a filename
    for it based on the following hacky rules:
    [1] spaces are converted to carrots ('^')
    [2] pipes are converted to dollar signs ('$')
    """
    tmp = cmd.replace(' ', '^')
    return tmp.replace('|', '$') + ".output"
...
def slurp_output_file(modname: str, cmd: str) -> str:
    ...
    """
    return Path(
        f"{TEST_OUTPUT_DIR}/{modname}/{get_command_output_filename(cmd)}"
    ).read_text()
...
def generate_output_for_commands(cmds: List[str], dirpath: str) -> None:
    ...
    in the directory specified, each under a different file. Each
    filename is generated with get_command_output_filename().
    ...
    os.makedirs(dirpath)
    for cmd in cmds:
        output_filename = get_command_output_filename(cmd)
        with open(f"{dirpath}/{output_filename}", 'w') as f:
